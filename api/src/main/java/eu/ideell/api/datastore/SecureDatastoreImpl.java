...
public class SecureDatastoreImpl extends AbstractSecureDatastore
  ...
  public LoadResult<Customer> loadCustomer(final User user) {
    return Monad.monad(user.getUserParent()).map(userParent -> userParent.getParent())
        .map(departmentParent -> departmentParent.getParent())
        .map(customerParent -> load().key(Key.create(Customer.class, customerParent.getName()))).get();
  }

  @Override
  public LoadResult<Department> loadDepartment(final User user) {
    return Monad.monad(user.getUserParent()).map(userParent -> userParent.getParent())
        .map(departmentParent -> load().key(Key.create(Department.class, departmentParent.getName()))).get();
  }
  ...
  @Override
  public <Entity> LoadResult<Entity> loadAuthorized(final User user, final Class<Entity> klass, final String name) {
    return null;
  }
